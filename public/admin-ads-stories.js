(() => {
  ...
  $('adSlideList').oninput=event=>{if(event.target.dataset.overlay!==undefined){slides[Number(event.target.dataset.overlay)].overlay_text=event.target.value;dirty=true;}if(event.target.dataset.duration!==undefined){slides[Number(event.target.dataset.duration)].duration=Number(event.target.value);dirty=true;}};
  ...
  $('adCampaignForm').onsubmit=async event=>{
    ...
    try{
      ...
      body.event_id=selectedEvent;body.active=body.active==='true';body.starts_at=body.starts_at?new Date(body.starts_at).toISOString():null;body.ends_at=body.ends_at?new Date(body.ends_at).toISOString():null;body.slides=slides.map(s=>({image_path:s.image_path,duration:s.duration,overlay_text:s.overlay_text||''}));
