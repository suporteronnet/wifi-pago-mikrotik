...
module.exports = function registerAdStories({app, db, adminAuth, requireRole, normalizeMac, adRouterHasClient}) {
  ...
  return {
    start:wrap((req,res)=>{
      ...
      const playlist=campaigns(event.id).flatMap(c=>c.slides.map(s=>({id:c.id,slide_id:s.id,name:c.name,target_url:c.target_url,button_label:c.button_label,whatsapp_url:c.whatsapp_enabled?`https://wa.me/${c.whatsapp_phone}?text=${encodeURIComponent(c.whatsapp_message)}`:null,image_path:s.image_path,duration:s.duration})));
