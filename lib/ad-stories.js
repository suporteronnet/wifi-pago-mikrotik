...
module.exports = function registerAdStories({app, db, adminAuth, requireRole, normalizeMac, adRouterHasClient}) {
  ...
  app.put('/admin/api/ad-portals/:id',...admin,wrap((req,res)=>{
    ...
    result.logo_path=String(body.logo_path||'');
    if(result.logo_path&&!/^\/api\/ad-images\/[a-zA-Z0-9-]+\.(jpg|jpeg|png|webp)$/.test(result.logo_path))return fail(res,400,'Envie uma imagem válida para a logo.');
  ...
  return {
    start:wrap((req,res)=>{
      ...
      // Resume only the same unexpired device/event session. Completion remains
      // server-owned: a page reload cannot skip Stories or the saved profile.
      if(/^[0-9a-f]{64}$/.test(String(body.resume_token||''))){
        const hash=crypto.createHash('sha256').update(body.resume_token).digest('hex');
        const saved=db.prepare(`SELECT v.*,r.playlist,r.settings_json,r.current_index,r.completed
          FROM ad_view_sessions v JOIN ad_story_runs r USING(token_hash)
          WHERE v.token_hash=? AND v.event_id=? AND v.router_id=? AND v.mac=? AND v.expires_at>?`)
          .get(hash,event.id,router.id,mac,new Date().toISOString());
        if(saved){
          const profile_saved=Boolean(db.prepare('SELECT id FROM ad_contacts WHERE token_hash=?').get(hash));
          return res.json({ok:true,token:body.resume_token,playlist:JSON.parse(saved.playlist),settings:JSON.parse(saved.settings_json),
            index:saved.current_index,completed:Boolean(saved.completed),profile_saved,access_requested:Boolean(saved.command_ref)});
        }
      }
