...
module.exports = function registerAdStories({app, db, adminAuth, requireRole, normalizeMac, adRouterHasClient}) {
  ...
  function campaigns(eventId, active=true) {
    return db.prepare(`SELECT a.id,a.name,a.image_path,a.target_url,a.active,a.starts_at,a.ends_at,COALESCE(s.duration,8) AS duration,COALESCE(s.position,0) AS position
      ...
      ORDER BY position,a.id`).all(eventId,eventId,...(active ? [new Date().toISOString(),new Date().toISOString()] : [])).map(c=>{
        const slides=db.prepare('SELECT id,image_path,duration,position FROM ad_campaign_slides WHERE campaign_id=? ORDER BY position,id').all(c.id);
        return {...c,slides:slides.length ? slides : [{id:null,image_path:c.image_path,duration:c.duration,position:0}]};
      });
  }
  ...
  function campaignBody(body,res){
    ...
    for(const slide of body.slides){
      ...
      slides.push({image_path:image,duration});
    }
    const dates={};
    ...
    return {name,target,position,slides,active:body.active===false?0:1,starts_at:dates.starts_at?.toISOString()||null,ends_at:dates.ends_at?.toISOString()||null};
  }
  function saveSlides(id,data){
    db.prepare('DELETE FROM ad_campaign_slides WHERE campaign_id=?').run(id);
    const insert=db.prepare('INSERT INTO ad_campaign_slides(campaign_id,image_path,duration,position) VALUES (?,?,?,?)');
    data.slides.forEach((s,i)=>insert.run(id,s.image_path,s.duration,i));
  ...
  return {
    start:wrap((req,res)=>{
      ...
      const playlist=campaigns(event.id).flatMap(c=>c.slides.map(s=>({id:c.id,slide_id:s.id,name:c.name,target_url:c.target_url,image_path:s.image_path,duration:s.duration})));
      if(!playlist.length)return fail(res,404,'Nenhum anúncio ativo neste evento.');
