  ...
  app.put('/admin/api/ad-portals/:id',...admin,wrap((req,res)=>{
    ...
    if(body.fields!==undefined){
      ...
      for(const f of body.fields){
        if(!f||!/^([a-z][a-z0-9_]{0,39})$/.test(f.id)||ids.has(f.id)||!['text','tel','email','textarea'].includes(f.type)||!String(f.label||'').trim()||String(f.label).length>120)return fail(res,400,'Revise os títulos e tipos dos campos.');
