...
test('mobile offer requests access and waits for ACK before opening destination',async()=>{
  for(const userAgent of ['Android','iPhone']){
    ...
    const context={connecting:false,profileSaved:true,token:'test',navigator:{userAgent},URL,Date,
      ...
      step:()=>{},message:()=>{},sleep:async()=>{},params:new URLSearchParams(),playlist:[],interests:new Set(),url:v=>v,
    ...
    await context.connectAccess('https://wa.me/5569999999999?text=Ola');
    assert.deepEqual(calls,['/api/ads/access','/api/ads/status','/api/ads/status','https://wa.me/5569999999999?text=Ola']);
  }
});
